...
impl ReferralRegistryContract {
    ...
    pub fn register_referral(
        ...
        let leaderboard: Address = env
            ...
            .unwrap();
        // Credit the welcome reward immediately as an optional side effect.
        // Registration remains successful if the leaderboard is paused,
        // unavailable, or exceeds the remaining invocation budget.
        ...
        match referrer {
            Some(ref_addr) => {
                ...
                let leaderboard: Address = env
                    ...
                    .get(&DataKey::LeaderboardContract)
                    .unwrap();
                // Credit points as an optional side effect. The XLM transfer
                // and earnings update remain critical referral accounting.
                // Try add_bonus_pts (immediate credit) first; fall back to
                // queue_bonus_reward (deferred) for older leaderboard builds.
                let _ = env.try_invoke_contract::<Val, soroban_sdk::Error>(
                    &leaderboard,
